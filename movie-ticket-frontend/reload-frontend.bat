@echo off
REM 重新部署前端容器以应用新的 nginx 配置
REM 使用方法: reload-frontend.bat
...
REM 停止并删除现有前端容器
echo 📦 停止现有前端容器...
docker stop cinema-frontend 2>nul
docker rm cinema-frontend 2>nul
...
echo.
echo 🔍 容器状态:
docker ps | findstr cinema-frontend
